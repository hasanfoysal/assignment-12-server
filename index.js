...
// const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.uivgn73.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0`;
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.uivgn73.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0`;
console.log(uri);
...
async function run() {
  try {
    ...
    const userCollection = client.db('productsHunt').collection('users')
    ...
    // const paymentCollection = client.db('productsHunt').collection('payments')
  ...
  app.get('/users/admin/:email', verifyToken,  async(req, res) =>{
    ...
    app.get('/trending', async(req, res) =>{
      ...
      const options = {
        sort: {
          vote_count: filter.sort === 'asc' ? 1: -1
        }
      };
    ...
    //   const query = { _id: {
    //     $in: payment.menuIds.map(id => new ObjectId(id))

    //   }}

    //   const deleteResult = await menuCollection.deleteMany(query);
